...
async def verify_db():
    ...
    async with async_session() as session:
        ...
        print(f"Found {len(logs)} prompt logs.")
        for log in logs:
            print(f"Log ID: {log.id}, Success: {log.success}, Latency: {log.latency_ms}, Error: {log.error_message}")
            print(f"Input Payload: {log.input_payload}")
